...
;(function(window, document, $, undefined)
{
	...
	Extension.Index.datePicker = function()
	{
		...
		$('.daterangepicker_start_input').attr('data-grid', 'status');

		$('.daterangepicker_end_input').attr('data-grid', 'status');
	...
	Extension.Index.dataGrid = function()
	{
		var config = {
			...
			events: {
				removing: function(dg)
				{
					...
					{
						if (filter.column === 'id' && filter.from !== undefined && filter.to !== undefined)
						{
							$('[data-grid-calendar]').val('');
						}
					});
		...
		Extension.Index.Grid = $.datagrid('status', '#data-grid', '#data-grid_pagination', '#data-grid_applied', config);
